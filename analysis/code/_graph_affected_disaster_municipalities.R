...
df <- df %>% 
  # Group data by municipality code and get the maximum year of the first landslide occurrence
  group_by(code) %>% 
  dplyr::summarise(first_year_landslide = max(first_year_landslide)) %>% 
  left_join(mun) %>%  # Merge with municipality data
  st_as_sf()  # Convert to an sf object for spatial operations
...
plot <- tm_shape(df) +
  tm_bubbles(col = "interval", scale = 0.5, n = 5, border.col = "white", 
             palette = "Spectral", title.col = "Year of the First Landslide Occurrence") +
  tm_shape(uf) +
...
tmap_save(plot, file = paste0(path_output_github, "graph_affected_disaster_municipalities_points.png"), 
